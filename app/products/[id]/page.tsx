...
export async function generateMetadata(
  ...
  const productImages = product.images.map((img: string) => 
    img.startsWith('http') ? img : `${process.env.NEXT_PUBLIC_APP_URL || 'https://arbooksellers.com'}${img}`
  )
  ...
  return {
    ...
    description: product.metaDescription || product.description?.substring(0, 160) || `Buy ${product.title} from AR Book Sellers. Authentic Islamic literature.`,
    keywords: [product.focusKeyword, 'Islamic books', 'Quran', 'Hadith', product.author].filter(Boolean),
    openGraph: {
      title: product.title,
