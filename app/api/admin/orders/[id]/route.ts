...
export async function PATCH(
  ...
  try {
    ...
    if (filteredData.orderStatus) {
      ...
      if (filteredData.orderStatus === 'delivered') {
        ...
        if (currentOrder && currentOrder.paymentStatus === 'failed') {
          return NextResponse.json({ 
            error: 'Cannot mark order as delivered when payment has failed. Please resolve payment issue first.' 
          }, { status: 400 });
        }
      ...
      if (filteredData.orderStatus === 'shipped') {
        ...
        if (currentOrder) {
          ...
          if (currentOrder.paymentMethod === 'cash_on_delivery') {
            ...
            if (!filteredData.paymentStatus && currentOrder.paymentStatus === 'pending') {
              console.log('Warning: Pre-paid order being shipped with pending payment');
            }
          }
        }
      }
    ...
    if (filteredData.orderStatus) {
      ...
      if (currentOrder) {
        if (filteredData.orderStatus === 'shipped' && 
          ...
          warnings.push('Warning: Pre-paid order is being shipped with pending payment');
        }
        
        if (filteredData.orderStatus === 'delivered' && 
            currentOrder.paymentMethod === 'cash_on_delivery' && 
            currentOrder.paymentStatus === 'pending') {
          warnings.push('Payment automatically completed for COD order upon delivery');
        }
      }
    }
    ...
    if (filteredData.orderStatus === 'delivered' && filteredData.paymentStatus === 'paid') {
      responseMessage = 'Order marked as delivered and payment completed automatically';
    } else if (filteredData.orderStatus === 'cancelled' && filteredData.paymentStatus === 'failed') {
